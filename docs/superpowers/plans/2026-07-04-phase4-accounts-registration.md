...
**Test**
- Modify `test/.../Children/ChildProfileAppService_Tests.cs`（改成按当前家长建娃）。
- Create 归属隔离测试（两个家长上下文）：`test/.../Accounts/OwnershipIsolation_Tests.cs`。
- Create `test/.../Children/ChildProfileManager_Tests.cs`。

---

## Chunk 0: Domain 改造 + 迁移

### Task 0.1: `ChildProfile` 改 `ParentId`（TDD）

**Files:** Modify `src/Homework.Domain/Children/ChildProfile.cs`；Modify `test/Homework.Domain.Tests/Children/ChildProfile_Tests.cs`。

- [ ] **Step 1: 改测试到新构造器（先红）** — 打开 `ChildProfile_Tests.cs`，把 `new ChildProfile(Guid.NewGuid(), Guid.NewGuid(), "哥哥", 3)` 的**第二个参数语义从 identityUserId 改为 parentId**（值不变，仍是个 Guid），并加一条：
```csharp
[Fact]
...
```
（原有校验姓名/年级的用例保持——构造器参数个数不变，仍能编过。）

- [ ] **Step 2: 跑红** — Run: `dotnet test test/Homework.Domain.Tests/Homework.Domain.Tests.csproj --filter "FullyQualifiedName~ChildProfile_Tests"` → 红（`ParentId` 不存在）。

- [ ] **Step 3: 改实体** — `ChildProfile.cs`：把 `public Guid IdentityUserId { get; private set; }` 改为 `public Guid ParentId { get; private set; }`；构造器参数 `identityUserId` 改名 `parentId`、赋值 `ParentId = parentId;`。其余不动。

- [ ] **Step 4: 跑绿** → 通过。

- [ ] **Step 5: 提交** — `refactor(children): ChildProfile.IdentityUserId -> ParentId (TDD)`。
...
### Task 0.3: `FamilyGoal` 加 `ParentId`（TDD）

**Files:** Modify `src/Homework.Domain/Scoring/FamilyGoal.cs`；Modify `test/Homework.Domain.Tests/...`（若有 FamilyGoal 构造测试）或加一条。

- [ ] **Step 1: 失败测试** — 加：
```csharp
[Fact]
public void FamilyGoal_Exposes_ParentId()
{
    var pid = Guid.NewGuid();
    var g = new FamilyGoal(Guid.NewGuid(), pid, "暑假", 100, new DateOnly(2026,7,6), new DateOnly(2026,7,12));
    g.ParentId.ShouldBe(pid);
}
```
- [ ] **Step 2: 跑红**。
- [ ] **Step 3: 实现** — `FamilyGoal.cs`：加 `public Guid ParentId { get; private set; }`；构造器**加第二参数 `Guid parentId`**（放在 `id` 之后、`title` 之前），赋值。其余不变。
- [ ] **Step 4: 跑绿**。
- [ ] **Step 5: 提交** — `feat(scoring): FamilyGoal.ParentId (TDD)`。

### Task 0.4: DbContext 映射 + 迁移

**Files:** Modify `HomeworkDbContext.cs`；生成迁移。

- [ ] **Step 1: 改映射** — `HomeworkDbContext.OnModelCreating`：
  - `ChildProfile`：把 `b.HasIndex(x => x.IdentityUserId).IsUnique();` 改为 `b.HasIndex(x => x.ParentId);`。
  - `FamilyGoal`：可选 `b.HasIndex(x => x.ParentId);`。
- [ ] **Step 2: 生成迁移** — Run（EF 工具见记忆 `abp-postgres-stack`，Design 已 pin 10.0.7）：
  `dotnet ef migrations add Reworked_Accounts -p src/Homework.EntityFrameworkCore -s src/Homework.EntityFrameworkCore`
  Expected: 迁移把 `AppChildProfiles.IdentityUserId` 改名/换成 `ParentId`（EF 可能识别为删列+加列，可接受，开发库数据可丢）、`AppFamilyGoals` 加 `ParentId`。
...
### Task 1.1: ChildProfile DTO/mapper/接口调整

**Files:** Modify `Children/Dtos/ChildProfileDto.cs`、`Children/IChildProfileAppService.cs`、`HomeworkApplicationMappers.cs`；Create `Children/Dtos/CreateChildDto.cs`。

- [ ] **Step 1:** `ChildProfileDto.cs` 去掉 `public Guid IdentityUserId { get; set; }`（`ParentId` **不加**进 DTO，不外泄）。
- [ ] **Step 2:** `HomeworkApplicationMappers.cs` 的 `ChildProfileMapper`：现在按名自动映射了 `IdentityUserId`；DTO 去掉该字段后，Mapperly 不再有对应目标，源属性 `ParentId` 无目标（Mapperly 对**未映射源**只告警不报错），无需额外 attribute。保留 `[MapperIgnoreTarget(nameof(ChildProfileDto.HasPin))]`。跑一次 `dotnet build src/Homework.Application` 确认无 error。
- [ ] **Step 3:** `CreateChildDto.cs`：
```csharp
...
```
- [ ] **Step 4:** `IChildProfileAppService.cs` 加：
```csharp
Task<ChildProfileDto> CreateAsync(CreateChildDto input);
Task DeleteAsync(Guid id);
```
- [ ] **Step 5:** 编译 → 成功。提交随 Task 1.2。
...
- [ ] **Step 4: 跑绿**（隔离用例 + 原有 Phase 2/3 用例；注意 Phase 2 `FamilyGoalProgress_Tests` 现在需要给 goal 传 parentId 且给 DailyScore 的 childId 归属到该 parent——相应调整这些既有测试）。
...
### Task 4.2: 端到端冒烟（`@run`）
- [ ] `cd src/Homework.DbMigrator && dotnet run`（迁移 + 示例家庭 + 角色/授权）。
