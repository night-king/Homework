...
**Goal:** 在仓库根 `frontend/parent-web/` 建一个 React SPA 家长后台，消费已完成的 headless API（`https://localhost:44394`），实现注册/登录、孩子档案、每周模板、每日看板(审阅/撤销)、家庭目标、首页汇总。
...
**Spec:** `docs/engineering/specs/frontend/2026-07-05-parent-console-design.md`

---

## Context & invariants（零上下文实现者必读）

- **仓库根有一个 .NET ABP solution**（`backend/src/ backend/test/ backend/Homework.slnx`）。本项目在**新目录 `frontend/parent-web/`**（与 `backend/`、`docs/` 平级），是**独立的 Node 前端**，**不进 `Homework.slnx`**、不被 .NET 构建。所有前端命令在 `frontend/parent-web/` 目录里跑（`cd frontend/parent-web && npm ...`）。
- **直接提交到 `main`**（单人项目惯例，无 feature 分支）。每个 Task 末尾 `git commit`；不 push。
- **后端已就绪且必须在跑**（联调/冒烟时）：`cd backend/src/Homework.HttpApi.Host && dotnet run` → `https://localhost:44394`；本地 PostgreSQL 在 `localhost:5433`，DbMigrator 已播种（demo 家长 `demo`/`1q2w3E*`、`Homework_App` 公共客户端）。dev 必须 HTTPS。
...
- **测试**：`cd frontend/parent-web && npx vitest run`（逻辑单测）；`npx tsc -b`（类型）；`npm run lint`。UI 不强制 TDD。

## File structure（frontend/parent-web/ 内）
...
目标：`frontend/parent-web/` 起得来、`tsc -b`/eslint/vitest 绿、Tailwind 温暖亲和主题生效。此 chunk 无领域逻辑。

### Task 1.1: 初始化 Vite React-TS 工程

**Files:** Create `frontend/parent-web/package.json`, `frontend/parent-web/index.html`, `frontend/parent-web/vite.config.ts`, `frontend/parent-web/tsconfig*.json`, `frontend/parent-web/vitest.config.ts`, `frontend/parent-web/.gitignore`, `frontend/parent-web/.env.example`, `frontend/parent-web/eslint.config.js`, `frontend/parent-web/src/vite-env.d.ts`, `frontend/parent-web/src/main.tsx`, `frontend/parent-web/src/App.tsx`, `frontend/parent-web/src/index.css`, `frontend/parent-web/src/lib/utils.ts`.

- [ ] **Step 1: 创建 `frontend/parent-web/package.json`**
...
- [ ] **Step 2: `frontend/parent-web/index.html`**
...
- [ ] **Step 3: `frontend/parent-web/vite.config.ts`**（dev 5173，代理到后端 44394）
...
- [ ] **Step 4: `frontend/parent-web/tsconfig.json` / `tsconfig.app.json` / `tsconfig.node.json`**
...
- [ ] **Step 5: `frontend/parent-web/vitest.config.ts`**
...
```
And create `frontend/parent-web/src/test-setup.ts`:
```ts
import '@testing-library/jest-dom/vitest'
```

- [ ] **Step 6: `frontend/parent-web/.gitignore`, `.env.example`, `src/vite-env.d.ts`**
...
- [ ] **Step 7: `frontend/parent-web/eslint.config.js`**（标准 vite react-ts flat config；可对标 port-shield 的 `eslint.config.js`）
...
Run:
```bash
cd frontend/parent-web && npm install
npx tsc -b
npm run dev
```
...
Create `frontend/parent-web/src/lib/utils.test.ts`:
```ts
...
```
Run: `cd frontend/parent-web && npx vitest run`
Expected: 1 passed.

- [ ] **Step 13: 提交**

```bash
cd frontend/parent-web && git add -A && cd .. && git add frontend/parent-web/.gitignore
git commit -m "feat(console): scaffold Vite React-TS + Tailwind4 warm theme + tooling"
```
> 根 `.gitignore` 已忽略 `node_modules`；确认 `frontend/parent-web/node_modules` 未被提交（`git status` 检查）。
...
**Files:** Create `frontend/parent-web/src/types/homework.ts`.
...
- [ ] **Step 2: typecheck** — Run `cd frontend/parent-web && npx tsc -b`；Expected 0 error。
- [ ] **Step 3: commit** — `git add frontend/parent-web/src/types && git commit -m "feat(console): homework DTO types"`

### Task 2.2: `src/services/api.ts`（axios + 401 refresh，TDD）

**Files:** Create `frontend/parent-web/src/services/api.ts`, Test `frontend/parent-web/src/services/api.test.ts`.
...
- [ ] **Step 3: 跑测试** — `cd frontend/parent-web && npx vitest run src/services/api.test.ts`；Expected: 3 passed（先确认 import 前 red：把 `getErrorMessage` 改名跑一次看 fail，再改回 green）。
- [ ] **Step 4: commit** — `git add frontend/parent-web/src/services/api.ts frontend/parent-web/src/services/api.test.ts && git commit -m "feat(console): axios client + 401 refresh interceptor + ABP error parsing"`

### Task 2.3: `src/services/authService.ts`

**Files:** Create `frontend/parent-web/src/services/authService.ts`.
...
**Files:** Create `frontend/parent-web/src/stores/authStore.ts`, Test `frontend/parent-web/src/stores/authStore.test.ts`.
...
**Files:** Create/replace `frontend/parent-web/src/i18n/config.ts`; Modify `frontend/parent-web/src/main.tsx`; Create `frontend/parent-web/public/locales/{zh-CN,en}/translation.json`.
...
- [ ] **Step 4: 验证** — `cd frontend/parent-web && npx tsc -b && npm run dev`，浏览器无报错、i18n 正常加载（控制台无 404 缺 locale）。
...
**Files:** Create `frontend/parent-web/src/components/ui/{button,card,input,label,textarea,select,switch,badge,separator,dialog,dropdown-menu,table,scroll-area}.tsx`.
...
- [ ] **Step 3: typecheck** — `cd frontend/parent-web && npx tsc -b`；Expected 0 error。
- [ ] **Step 4: commit** — `git commit -m "feat(console): shadcn-style UI primitives themed warm"`

### Task 3.2: `AppLayout` + `App.tsx` 路由

**Files:** Create `frontend/parent-web/src/components/layout/AppLayout.tsx`; Modify `frontend/parent-web/src/App.tsx`; Create `frontend/parent-web/src/components/{LanguageSwitcher,UserMenu}.tsx`.
...
**Files:** Create `frontend/parent-web/src/features/auth/{LoginPage,RegisterPage,ForgotPasswordPage,ResetPasswordPage}.tsx`.
...
**Files:** Create `frontend/parent-web/src/services/homeworkService.ts`.
...
**Files:** Create `frontend/parent-web/src/components/ConfirmDialog.tsx`, `frontend/parent-web/src/hooks/useChildren.ts`, `frontend/parent-web/src/features/children/{ChildrenPage,ChildFormDialog,SetPinDialog}.tsx`.
...
**Files:** Create `frontend/parent-web/src/hooks/useWeeklyTemplates.ts`, `frontend/parent-web/src/features/schedule/{WeeklyTemplatePage,TemplateItemDialog}.tsx`.
...
**Files:** Create `frontend/parent-web/src/hooks/useDailyBoard.ts`, `frontend/parent-web/src/features/board/{DailyBoardPage,DailyTaskDialog}.tsx`, `frontend/parent-web/src/components/StarRating.tsx`.
...
**Files:** Create `frontend/parent-web/src/components/ProgressBar.tsx`, `frontend/parent-web/src/hooks/useGoals.ts`, `frontend/parent-web/src/features/goals/{FamilyGoalsPage,GoalFormDialog}.tsx`.
...
**Files:** Create `frontend/parent-web/src/features/home/HomePage.tsx`.
...
**Files:** Create `frontend/parent-web/src/features/account/{ProfileDialog,ChangePasswordDialog}.tsx`; Modify `frontend/parent-web/src/components/UserMenu.tsx`.
...
- [ ] **Step 2: 全量检查** — `cd frontend/parent-web && npx tsc -b && npm run lint && npx vitest run`；Expected：0 type error、lint 通过（warning 可接受）、全部单测 passed。
...
- [ ] `cd frontend/parent-web && npm run dev` 起在 5173；`npx tsc -b`、`npm run lint`、`npx vitest run` 全绿。
