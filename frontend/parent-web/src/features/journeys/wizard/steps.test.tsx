...
describe('StepBasics', () => {
  it('patches title and shows date-order error', () => {
    const patch = vi.fn()
    const state = { ...emptyWizardState(), startDate: '2026-08-01', endDate: '2026-07-01' }
...
describe('StepMedal', () => {
  it('selects a medal on click', () => {
    ...
    render(<StepMedal state={emptyWizardState()} patch={patch} medals={medals} />)
  ...
  it('shows empty hint when no medals', () => {
    render(<StepMedal state={emptyWizardState()} patch={vi.fn()} medals={[]} />)
...
describe('StepReview', () => {
  it('summarizes title, medal name and task count', () => {
    const state = { ...emptyWizardState(), title: '暑假', medalId: 'm1', startDate: '2026-07-01', endDate: '2026-08-31',
