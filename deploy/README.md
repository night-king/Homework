...
域名：**app.homework.today**。Cloudflare SSL 用**灵活(Flexible)**：边缘对访客是 https，回源走 http:80，**源站只监听 80、不需要证书**。
...
1. DNS 把 `app.homework.today` 的 A/AAAA 记录指向服务器 IP，**代理开启（橙云）**。
...
sudo cp deploy/nginx/homework.conf /etc/nginx/sites-available/homework   # 已按 app.homework.today + 灵活模式(80) 配好
