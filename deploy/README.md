# 裸机部署（Ubuntu + supervisor + nginx + .NET 10 runtime，Cloudflare 小橙云在前）

面向单实例小流量自部署。前端是**静态包**（无 Node 常驻），真正的服务只有 **PostgreSQL + ABP API 宿主**。

```
浏览器 ──https──> Cloudflare(橙云,边缘缓存/TLS) ──https──> nginx(源站,443) ┬─ 静态 dist/ (家长端)
...
- **反代 https 识别**：Host 已加 `UseForwardedHeaders`（读 `X-Forwarded-Proto`），否则 nginx 终止 TLS 后 OpenIddict 判定非安全传输、拒绝 `/connect/token`。nginx 配置里已带该头。
- **资产 /blob**：`/blob/{key}` 端点已放开到生产，配合 `AssetCdnBaseUrl=https://域名/blob` 同域名出图（CF 会缓存）。想用 OSS 就把 `Aliyun` 配置填上、`AssetCdnBaseUrl` 指到 OSS/CDN，此端点自动闲置。

> 改了 Host 代码后**必须重新 publish**（见本仓库根 `output/`）。发布命令：
...
1. DNS 里把域名 A/AAAA 记录指向服务器 IP，**代理开启（橙云）**。
2. SSL/TLS → 概览：模式选 **Full (strict)**。
3. SSL/TLS → Origin Server → **Create Certificate**，把证书存服务器 `/etc/ssl/cloudflare/homework.pem`、私钥 `/etc/ssl/cloudflare/homework.key`（`chmod 600`）。
4. （可选）缓存 → 页面规则：对 `你的域名/api/*`、`/connect/*` 设 **Bypass Cache**，避免误缓存接口。
5. （可选，安全）源站防火墙只放行 Cloudflare IP 段（`ufw` 或云安全组），杜绝绕过 CF 直连。
...
sudo mkdir -p /var/www/homework/{api,dbmigrator,web} /var/log/homework /etc/ssl/cloudflare
...
- `App:SelfUrl`、`App:CorsOrigins` → `https://你的域名`。
- `App:AssetCdnBaseUrl` → `https://你的域名/blob`（同域名出图）。
...
# 把 homework.conf 里的 your.domain.com 换成真实域名
...
- `https://你的域名/` → 家长端加载、能登录（`admin` / `1q2w3E*` 或 `demo` / `1q2w3E*`，见根 DEPLOY.md）。
- 登录后能创建旅程、设置孩子 PIN；进孩子模式 PIN 生效。
- `https://你的域名/swagger` 可访问（如不想暴露可在 nginx 屏蔽 `/swagger`）。
...
见仓库根 `DEPLOY.md`（HTTPS/证书、密钥移出明文、注册防滥用、邮箱验证、家长同意服务端校验、生产错误页、安全响应头、儿童数据合规）。本目录只覆盖“把它跑起来”，加固项请务必逐条过。
